...
void stop_alarm() {  //para parar o ll
   
    TIMEOUT = TRUE;
}

void start_alarm(int sec) {  //para começar o ll
...
int send_frame(unsigned char* frame, int size, int fd) {  //enviar a trama para download do tux
...
int receive_frame(unsigned char* frame, int fd) {  //receber a trama 
...
int llwrite(char* buf, int bufSize) {
    ...
    while(STOP == FALSE) {
        ...
        //Caso a trama recebida corresponda à trama REJ0, reenvia a trama I
            ...
            printf("REJ0 recebida\n");

            send = send_frame(frame, frameSize, fd);
            if(send < 0) {
                return -1;
            }
            printf("trama reenviada\n");
        } 
        ...
        //Caso a trama recebida corresponda à trama REJ1, reenvia a segunda trama I
            ...
            printf("REJ1 recebida\n");

            send_frame(frame, frameSize, fd);
            printf("segunda trama reenviada\n");
        }
    }
